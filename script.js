...
guiDisplay
  .add(displayParams, "showFormula")
  .name("📖 Hiển thị công thức")
  .onChange((value) => {
...
// ---------- Ánh sáng scene ----------
if (!scene.light) {
  scene.light = new THREE.DirectionalLight(
...
function updateLightMode() {
  ...
  // --- Cập nhật ánh sáng ---
  if (scene.light) scene.remove(scene.light);

  switch (lightParams.lightType) {
    case "Point":
      scene.light = new THREE.PointLight(
        lightParams.lightColor,
        lightParams.lightIntensity
      );
      scene.light.position.set(50, 50, 50);
      break;
    case "Spot":
      scene.light = new THREE.SpotLight(
        lightParams.lightColor,
        lightParams.lightIntensity
      );
      scene.light.position.set(50, 100, 50);
      scene.light.angle = Math.PI / 6;
      break;
    default:
      scene.light = new THREE.DirectionalLight(
        lightParams.lightColor,
      ...
      scene.light.position.set(60, 60, 60);
  }
  scene.add(scene.light);
...
function animateLights(deltaTime) {
  if (lightParams.animateLight && scene.light) {
    const t = Date.now() * 0.001;
    scene.light.position.x = 60 * Math.sin(t);
    scene.light.position.z = 60 * Math.cos(t);
  }
}
...
function visualizeIntegral1D3D() {
  ...
  const old = scene.getObjectByName("integral1DSurface");
...
const drawParams = {
  ...
  clearAll: () => {
    ...
    drawParams.lines.forEach((l) => {
      scene.remove(l.line);
      if (l.lengthSprite) scene.remove(l.lengthSprite);
...
function onMouseClick(event) {
  ...
  if (intersectsArrow.length > 0) {
    ...
    if (!selectedVectors.includes(vObj)) {
      ...
      vObj.arrow.setColor(new THREE.Color(0xffff00)); // chọn -> vàng
    } else {
      // Nếu đã chọn rồi thì click lần nữa để bỏ chọn
      selectedVectors = selectedVectors.filter(v => v !== vObj);
      vObj.arrow.setColor(new THREE.Color(0x00ff00)); // bỏ chọn -> xanh
    }
    return; // ⬅️ có chọn thì thoát
  }
...
calcFolder.add(vectorParams, "scalarValue", -10, 10, 0.1).name("🔢 Hệ số nhân");
calcFolder.add(vectorParams, "scalar").name("📈 Nhân với số");
calcFolder.add(vectorParams, "proj").name("📍 Chiếu v1 lên v2");
...
calcFolder.add(vectorParams, "triple").name("📦 Thể tích khối hộp");

calcFolder
  .add({ rel: checkVectorRelations }, "rel")
  .name("🔎 Quan hệ 2 vector");
calcFolder
  .add({ dist: distanceBetweenVectors }, "dist")
  .name("📏 Khoảng cách 2 vector");
...
// ===== Xóa vector khỏi scene và lịch sử =====
function removeVector(vObj) {
  scene.remove(vObj.arrow);
  if (vObj.dashedLine) scene.remove(vObj.dashedLine);
...
function addPoint(pos) {
  ...
  const label = makeTextSprite(`(${pos.x.toFixed(1)}, ${pos.y.toFixed(1)}, ${pos.z.toFixed(1)})`);
  label.position.copy(pos.clone().add(new THREE.Vector3(0.2, 0.2, 0.2)));
  scene.add(label);

  drawParams.points.push({ mesh, label, pos });
...
function tripleProductFromHistory() {
  ...
  if (window.parallelepipedMesh) {
    scene.remove(window.parallelepipedMesh);
  }
